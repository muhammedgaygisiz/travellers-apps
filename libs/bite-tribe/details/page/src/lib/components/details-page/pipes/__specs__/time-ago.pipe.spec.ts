...
describe('TimeAgoPipe', () => {
  ...
  it('should return "7 d ago" for a week ago', () => {
    const weekAgo = new Date('2025-06-02T12:00:00Z');
    expect(pipe.transform(weekAgo.toISOString())).toBe('7 d ago');
  });

  it('should return "30 d ago" for a month ago', () => {
    const monthAgo = new Date('2025-05-09T12:00:00Z');
    expect(pipe.transform(monthAgo.toISOString())).toBe('31 d ago');
  });
  ...
  it('should handle same day (less than 24 hours)', () => {
    ...
    expect(pipe.transform(sameDay.toISOString())).toBe('1 d ago');
  });

  it('should handle invalid date string', () => {
    const invalidDate = 'not-a-date';
    expect(pipe.transform(invalidDate)).toBe('NaN d ago');
  });
});
