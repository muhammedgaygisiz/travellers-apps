...
- [Current State - Known Issues](../current-state/known-issues.md) keeps the profile row for the [issue #1188][#1188] visibility verification only. The row is renamed from "Profile sharing" to "Profile visibility" and states that the absent share flow is intended, so the entry cannot be read as an open launch risk again.
- The boundary is written into the pages that own it, so it does not have to be rediscovered from this issue: an "Out Of Scope" section in [UC - Manage Profile And Social Graph](../use-cases/uc-manage-profile-and-social-graph.md), a business rule in [User](../domain/user.md), the shareable-entity rule in [Bite](../domain/bite.md), and the Bite-only sharing scope in [UC - Inspect Bite Details](../use-cases/uc-inspect-bite-details.md).
