...
- **A destructive confirmation's confirming button plays the `warning` haptic, and nothing else in the alert does.** The three alerts that guard a destructive action - delete Bite in `libs/bite-tribe-common/bite`, and unfollow in `libs/bite-tribe/followers/page` and `libs/bite-tribe/profile/page` - each fire `HapticsService.warning()` from their `didDismiss` handler, inside the branch that already checks for the confirming role. Opening the alert and cancelling it stay silent, because a haptic on either would make the tap that commits nothing feel exactly like the tap that commits. `warning` (Heavy impact) is reserved for this and is not used anywhere else in either app. It is fired, not awaited, and `HapticsService` never rejects, so the delete or unfollow it precedes cannot be blocked by it. Deleting a Bite also raises a success toast, which plays its own `success` intent through `ToastService`; that is not a duplicate but two intents at two different moments, the tap and the outcome. Unfollowing raises no toast, so the confirming tap is the only feedback it has. See GitHub issue [#1636].
- **Feedback that has no toast and no Ionic overlay behind it has to fire its own haptic, from the tap rather than from the outcome.** Reacting to a Bite goes through `LikesComponent` in `libs/bite-tribe-common/bite`, which dispatches `saveLike` / `removeLike` and raises nothing: no toast carries a `success` intent for it, and the reaction chip is an `ion-chip` in a popover, not a component Ionic gives haptics of its own. So `HapticsService.selection()` is played at the top of `onLikeSelected`, above the branch that picks save or remove, which is what makes it fire once per tap in either direction and once - not twice - when switching reaction type. Firing it from the store's eventual success instead would put the feedback behind a network round trip on an optimistic UI, and would go silent exactly when the write fails, which is the moment the tap most needs an answer. Opening the popover stays silent: choosing is not yet reacting. See GitHub issue [#1637].
...
- **A native `input[type="time"]` cannot be pinned to one width.** Chrome takes the format for the control from the **browser's** UI locale, not the document's, and ignores `lang` on the input or any ancestor - so the same stored `15:00` lays out as `15:00` in a 24-hour locale and `03:00 PM` in a 12-hour one. `bt-opening-hours` sized its two boxes at a flat `110px`, which is the 24-hour natural width to the pixel (78px of control plus the 16px inline padding Ionic's outline fill adds on each side), so in a 12-hour locale the day period was clipped away and `18:00` read back as `06:00` - indistinguishable from a real `06:00`, and `00:00` from `12:00`. Give such a box `min-width` sized for the 12-hour form and `width: auto` beside it. The floor holds the common case; `width: auto` is what lets a longer day period (`a. m.`, `午前`) grow the box instead of losing characters, and it is required rather than implied, because `ion-input` is `width: 100%` by default and as a flex item that resolves against the whole row rather than the control's own intrinsic width. See GitHub issue [#1549].
...
[#1250]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1250
[#1251]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1251
