...
3. **[#1184](https://github.com/muhammedgaygisiz/travellers-apps/issues/1184): Push notification settings cannot be changed.** The implementation now makes the product preference actionable, reports the live OS permission separately, handles an unspent prompt and denied recovery, prevents login-time registration while opted out, and filters explicit opt-outs before backend token delivery. Physical iOS and Android re-verification is still required before this charter item passes.
