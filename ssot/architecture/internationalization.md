...
- Text derived from a value rather than authored — a relative timestamp, a region name, a currency name, a date range — belongs to `Intl`, not to Transloco. A translation key can only carry copy someone wrote; the plural rules, unit abbreviations, and word order these need are already defined per language by CLDR. `Intl` also settles distinctions a blind translation loses: hand-abbreviating minutes to `min` and months to `m` was unambiguous only in English (issue [#1272]).
- A pure pipe is re-evaluated when one of its arguments changes, and the active language is not one of them unless it is passed in. A pipe that reads `TranslocoService` itself keeps rendering the language that was active when the view was created, which `reRenderOnLangChange` does not fix — that setting re-renders `TranslocoPipe`, not every pipe in the view. Pipes that localize take the language as an argument, from the component's `activeLang` signal.
