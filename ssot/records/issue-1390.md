# issue-1390 - bug: Offline photo upload retry never times out

Closing record for [#1390].
Written before the [Closing Report](../overview/github-issue-format.md#closing-report) rule; kept as-is, not extended.

## Description

- Found in [Test Run 09 - Android Build 95](../test-runs/09-android-build-95.md), the Android first execution, on Play Open Testing build 1.0.1 (95), on a physical Samsung SM-A566B running Android 16.
...
- **Recorded as an architecture rule, not a Bite rule.** Awaiting a Firestore write on an offline-capable path is a data-access mistake any feature can repeat, so it belongs on [Architecture - Data Access](../architecture/data-access.md).
...
- Recorded as the Offline Write Rule on [Architecture - Data Access](../architecture/data-access.md).
...
- Recorded as a rule on [Bite](../domain/bite.md).
