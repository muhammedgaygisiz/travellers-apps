...
| Android  | Samsung SM-A566B (Galaxy A56) | 16 (SDK 36) | Physical            | Play Open Testing build 95, run 9 executed              |
...
**The most likely root cause of the misidentification, worth checking first because it is a one-line fix.** On Android, `FirebaseUser.getProviderData()` includes the Firebase user record itself as the first element, with `providerId` of `firebase`, before the real providers; iOS and the web SDK do not. `getProviderId` reads index 0 unconditionally, so on Android it returns `firebase`, which is unmapped, giving `unknown`. That matches every observation: constant on Android, still wrong immediately after a fresh login, and correct on iOS. The `?? 'password'` fallback cannot save it, because index 0 is present with the wrong value. Confirm by logging the whole `providerData` array from `FirebaseAuthentication.getCurrentUser()` on a device.

**Still unexplained: the inside-window failure.** Three candidates, cheapest first - the call never reaches the server (no new function-log entry at all); it reaches the server and `deleteAccountForUser` fails (`account deletion failed` logged); or `auth_time` is not refreshed by the native `signInWithEmailAndPassword`, which would make the window unsatisfiable on Android and fold both symptoms into one auth-state problem. One attempt inside the window with the function log read immediately afterwards separates them.
