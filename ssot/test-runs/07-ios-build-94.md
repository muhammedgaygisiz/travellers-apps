...
Execution - Build 94 (Run 7)" of [Current State - Release Candidate Test Charter](../current-state/release-candidate-test-charter.md),
...
- **Build provenance is known before execution, for the second run running.** Build 94 is source commit `4fdf021d`, tag `build-1.0.1-94`, changelog range `a2753b18..4fdf021d`, recorded in release PR [#1324]. The tag points at the bump commit rather than the released source, which is the standing gap in [Release Workflow](../overview/release-workflow.md), so `4fdf021d` is the SHA to trust.
...
- **The single Run-6 blocker is fixed in this build, and it is the priority of this run.** [Issue #1308][#1308], reviews publishing the author's legal name, is closed against PR [#1312]. Read the fix before testing it, because its shape decides what counts as evidence:
...
- **Build 94 carries fixes for seven of the eight issues Run 6 filed.** [#1303] version `0.0.0` (PR [#1314]), [#1304] the deleted-Bite alert outliving its page (PR [#1315]), [#1305] toast position and failure signalling (PR [#1316]), [#1306] the position source modal (PR [#1320]), [#1308] as above (PR [#1312]), [#1309] the reply field (PR [#1321]), and [#1310] the stale profile Bite count (PR [#1322]). Closure state on GitHub does not replace physical verification — the rule that caught [#1232] in Run 5 and refuted [#1265] in Run 6.
- **Two issues are knowingly not fixed in this build**, and must not be re-filed as new findings:
  - [#1307], restaurant suggestions drawn from the device position rather than the Bite's, is open and no fix is in this changelog range. Expect it to reproduce.
  - [#1265], the verification mail sender, is open and was reclassified in Run 6 as a configuration issue rather than a code one. Nothing in build 94 changes it.
...
- **[Issue #1303][#1303] is verified on all three surfaces it touched.** The app menu reads `Version 1.0.1 (Build 94)`; the About page reads the same, so the version resolves consistently in both places rather than one being fixed and the other not; and the user document in Firestore holds `appVersion: "1.0.1"` with `appBuildNumber: "94"`.
...
- Incidental evidence for [issue #1296][#1296]: search located this Bite **1174 km from the device** by place name, with no proximity assumption. The review timestamp rendered as the localized relative string `letztes Jahr`.

## Session 3 - [#1309], The Reply Field

- The reply field held **three full lines** of typed text with all of it visible, the box having grown to fit. [Issue #1309][#1309] was that the field showed almost nothing, so this is a clean pass.
...
- **[Issue #1307][#1307] reproduced on build 94**, as the entry state expected. The tester observed the suggestion list drawn from around the **device** rather than the Bite while creating the Ronda Bite, which is the behaviour Run 6 characterised: the candidate list comes from the device's surroundings and is then ranked by distance from the Bite, so for a Ronda Bite it can only offer Bern restaurants over 1500 km away. The restaurant was ultimately found by typing its name, which is the manual compensation the in-app hint asks for. Recorded without an artifact — the observation was made before the capture was taken — and the issue stays open with no fix in this changelog range.
- Saving produced a **green success toast at the top of the screen**, `Bite erfolgreich erstellt!`, which auto-dismissed after roughly five seconds, after which the app returned to the Bites list with no residual alert. This covers the success half of [issue #1305][#1305] only.
- **[Issue #1310][#1310] is verified.** The Firestore baseline before the save was `biteCount: 606`; the profile read **607 immediately afterwards with no app restart**, which is exactly the behaviour the issue said was missing. The created Bite listed as `1539 KM`, `RONDA, SPANIEN`, confirming the position landed on the Google place rather than the device.
...
- **The delete toast was green and reported success**, observed directly by the tester. It is recorded without an artifact: the capture burst's window elapsed before the action was performed and all frames were byte-identical, so the screenshot evidence for this one observation does not exist. The cause was procedural rather than an app behaviour — the burst was started and _then_ the tester was told to act, so the window closed while the instruction was still being read. Future timing-sensitive captures should use an audible cue and a window measured in minutes. With the create and delete toasts both green on success, [issue #1305][#1305]'s success path is covered on two distinct actions; its failure path remains untested.
- Opening the deleted Bite's surviving photo from the local gallery produced a **blocking modal**, `Bite nicht gefunden` / `Dieser Bite ist nicht mehr verfügbar. Er wurde möglicherweise gelöscht.`, with a `Zurück` action and skeleton placeholders behind it rather than a hang. [Issue #1232][#1232]'s not-found branch remains fixed on build 94.
- **[Issue #1304][#1304] is verified.** `Zurück` dismissed the alert, returned to the gallery, and other photos opened normally. The alert neither outlived its page nor left the gallery unresponsive behind it.
...
- **The Run-6 blocker is resolved and verified.** [#1308] was the single P0 that made Run 6 a release-candidate fail, and the fix is confirmed on a review written a year before it. Build 94 carries **no inherited P0**.
...
[#1312]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1312
[#1314]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1314
[#1315]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1315
[#1316]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1316
[#1320]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1320
[#1321]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1321
[#1322]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1322
[#1324]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1324
