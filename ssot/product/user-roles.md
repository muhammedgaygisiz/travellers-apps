...
| **Table Guest**        | Tischgast              | `tableGuest` | A guest holding the anonymous session a scanned table code mints ([#1101]). A role since issue [#1629], and the only one **nobody grants**: `startTableSession` writes the claim on the account it has just admitted, `setUserRoles` refuses it outright, and the admin app's picker offers the three an operator can actually give. It grants nothing - every gate reads the sign-in provider rather than this claim (`RD-TS-40`) - and what it buys is that a table guest is nameable: in the session lists of the business and admin apps, and in the `Actors` of the use case, which admits roles. `RD-UR-9` | Table guest                             |
...
| **Table Guest**               | **Moved into the roles table above by issue [#1629].** It was listed here for as long as it carried no claim; it now carries `tableGuest`, written by the scan that mints the session. What has not changed is everything else about it: it leaves no `/users` document, it passes no route guard, and it opens nothing - which is why the decision that made it a role also narrowed what a role means (`RD-UR-9`) |
...
**A fourth thing the three categories did not cover, and the reason the third was narrowed
on 20 September 2026: the kind of session.** The table guest of issues [#1101] to [#1108] is
not a persona, because removing it changes what the table callables admit, and not an
entitlement, because nothing is recorded about it anywhere. It was not a role either while a
role meant something an operator grants - and issue [#1629] decided the other way round: it
_is_ a role, and what a role means now admits one the system writes and nobody hands out
(`RD-UR-9`).
