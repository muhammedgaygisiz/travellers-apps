...
- How does a real restaurant that never reaches five Bites become a Restaurant at launch? Automatic detection requires five matching Bites within 200 m before it proposes anything, so in a thin market it never fires. The Operator's on-demand clustering callable applies no threshold and is the intended answer, but it is classified `[Secondary]` in [[UC - Detect Restaurant Candidate]] and is therefore not in the initial release. That leaves the Admin App's Bite-places creation path as the only route - and it seeds `position: {latitude: 0, longitude: 0}`, creates no Candidate, and writes a Restaurant through five client-side writes with no transaction and no backend validation. Both halves are open: whether that path is retired, and what covers the thin-market case if it is. Epic \#1495 explicitly declines the `{0, 0}` defect, and epic \#1523 declines the decision, so nothing owns it today. Recorded 8 September 2026, from `E13` on that page.
