...
- Can a maintainer put their own tags on a restaurant? Nothing of the kind exists: there is no tag field on `Restaurant` and nothing in the business app writes one. The tags both restaurant pages already render are derived from Bites and belong to [UC - Browse Restaurants And Places](../use-cases/uc-browse-restaurants-and-places.md); a maintainer-set tag would be the restaurant describing itself rather than its diners describing it. It was the last idea left on the use case that carried "restaurant profile completeness", and it is recorded here because that page was deleted rather than because the idea was dropped. If it is wanted it needs an owning use case and a field; if not, nothing in the SSOT should describe it.
- What are the brand characters called, and what does a surface use one for? The seven committed in [issue 1482](https://github.com/muhammedgaygisiz/travellers-apps/issues/1482) carry the design export's filenames, which mix a historical term with national ones, and nothing in the product references them. Both halves are open: the vocabulary, and whether a character becomes an avatar, a BiteTrail marker, an empty state, or stays brand material. See [Implementation - Brand Characters](../implementation/brand-characters.md).
- Where does Leela appear? She is named as BiteTribe's main character and her reference artwork is cut from the street food comic, but only the comic uses her. Open: whether she stays a campaign character on social, or becomes the face of onboarding, empty states or the store screenshots - and the clean redraw that any of those needs. See [Implementation - Brand Characters](../implementation/brand-characters.md#leela).
...
- Can guests order without a BiteTribe account? **Settled 13 September 2026 by `RD-TS-4`, reopened on 17 September by issue [#1629] and closed again on 20 September by the same issue:** yes, through anonymous authentication. The holder of that session is now a _role_ rather than a description - `tableGuest`, written by the scan and granted by nobody (`RD-UR-9`) - which is what let the use case name its actor. A guest who registers during the meal keeps the session (`RD-TS-41`, issue [#1657]); one who signs into an account they already had takes the meal with them (`RD-TS-42`, issue [#1658]).
- **What may an anonymous session reach beyond the table it scanned? Settled 20 September 2026 by `RD-TS-37` and `RD-TS-38`, delivered by issue [#1565].** Nothing: `isMember()` takes the fifteen read clauses written for members, and the nineteen member callables refuse a token whose sign-in provider is `anonymous`. A uid minted by photographing a sticker reads its own table and no `/users` document, no Bite and no menu.
- **Does revoking or rotating a table code end the sessions it minted? Settled 20 September 2026 by `RD-TS-39`:** no. Revocation stops the next scan; a party already ordering keeps its session until the visit closes or it goes idle. Closing the visit is the floor's remedy for a party that should not be there.
- **What may one member read about another?** Untouched by the above and still open: `/users` and its follow graph stay readable by any member. Issue [#1611] narrows the leaderboard half - the cached email addresses - and nothing owns the rest.
...
- **How are table changes handled after ordering has started? Settled 20 September 2026 by `RD-TS-43`, delivered on issue [#1072].** The number follows the party and the ticket keeps its own: `moveTableVisit` stamps `currentTableId` and `currentTableLabel` onto the sessions and orders of the visit, the guest's screen and the kitchen queue read the destination, and `tableId` goes on naming where the scan and the order happened - so the waiter walks to the right table and the pass still recognises its own ticket. A queue group holding orders from more than one origin names none.
- How are unavailable menu items communicated? **Settled.** Issue [#1100] answered the scan - a menu whose every dish is off resolves to `menuUnavailable` rather than to an empty screen, and issue [#1597] narrowed that to the scans an order was otherwise possible on. Issue [#1102] marked individual items in the browsing surface. Issue [#1103] closed the last gap: an unavailable dish keeps a visible row with a **disabled** add button rather than disappearing, because a dish that vanishes when the kitchen runs out reads as a menu that changed, and the cart and the backend both refuse it by name if a second entry point gets past the button.
- **Does a guest order the extras a menu offers, and where do those extras live? Settled 20 September 2026 by `RD-TS-35` and `RD-TS-36`, delivered by issue [#1598].** `Category.extrasBlock` had existed since before stage 3 with nothing rendering it, so the first half of the question was whether the block belongs on the category at all - "extra cheese" reads as a property of a pizza rather than of the Pizze section. It stays on the category, because the block's own description is the owner's sentence saying who the extras are offered with, which is what a paper menu prints once under a section; a variant takes its dish's extras. The second half is that an extra is now orderable on the same terms as a dish: its own id, a per-line snapshot of name and price, revalidation against the live menu, and two refusals of its own. What is deliberately still not expressible is an extra offered on only _some_ items of a category, and any option group with a cardinality rule - "choose one base" is a different model, out of scope on the issue and unowned.
...
- **What should a scan at a menu-only restaurant do? Settled 13 September 2026 by `RD-TS-6`, delivered by issue [#1102].** It shows the way to the menu. `tableOrderingDisabled` and `orderingPaused` left the refusal list and became an ordering verdict on a scan that resolved; `restaurantClosed` stayed a refusal, because it is a statement about the restaurant rather than about ordering. Issue [#1597] finished the branch: `menuUnavailable` no longer refuses a menu-only scan either, because nothing was going to be ordered there.
- **Where do staff see a pending table session? Settled and delivered.** Issue [#1101] wrote the signal and issue [#1107] drew it: the order queue carries the waiting parties, one row per table, longest wait first. Issue [#1629] added the register beside it - every guest attached to a table, ended sessions dropped - and gave the operator the same question across every restaurant in the admin app (`RD-TS-44`).
- **Is staff confirmation mandatory or configurable? Settled by what shipped, and recorded here on 20 September 2026.** `RD-TS-1` made it unconditional - a scan at an unseated table is always `pending` - and nothing in the workspace configures it: issue [#1107]'s scope line proposing a per-restaurant switch was never built, and no restaurant document carries such a field. A restaurant that wants scans to seat parties by themselves is a new decision rather than a setting somebody forgot to wire up.

Visit summary and Bites, stage 4, issue [#1073]:

- **Does BiteTribe hold or route funds, or is payment always settled by the restaurant? Settled 21 September 2026 by `RD-TS-45`, recorded as [ADR-0004 Table Payment Model](../decisions/adr-0004-table-payment-model.md).** The restaurant settles. BiteTribe integrates no provider, touches no card data, holds and routes no funds, and issues no payout or refund - and `TableVisit.paymentStatus` therefore collapses from a provider's five states to `unsettled` and `settled`, written by staff. The scope is **table settlement only**: the monetization stages of issue [#1121] still sell through the app stores, where the store is merchant of record, and this settles nothing about them.
- **Which market is first, and what receipt and fiscalisation rules apply there? Dissolved by the answer above, and recorded as `RD-TS-46`.** What BiteTribe produces is a **visit summary** and never a receipt or an invoice: it makes no fiscal claim, so there is no first payment market to choose and adding a country costs nothing in this flow. The wording is part of the contract rather than a label - a surface that prints a tax line or numbers the document sequentially has made the claim the decision says BiteTribe does not make.
- **How is the summary delivered, and what does it cost a guest in personal data? Settled 21 September 2026 by `RD-TS-46`.** In the app, and optionally by email - asked once, at the **end** of the visit, where declining costs nothing. Asking at the scan was refused: it puts a personal-data question in front of a meal rather than after one. The address is used for that one send and stored nowhere, so the flow needs no retention rule and no deletion path for a uid nobody can sign back into. An anonymous guest keeps the in-app summary until their session expires (`RD-TS-5`), and a registered one keeps it permanently.
- **Does the guest see their own bill or the table's? Settled 21 September 2026 by `RD-TS-47`.** Both, and the table's names nobody. `RD-TS-12` already gives a guest the orders their own phone sent; the shared bill it deferred is a **backend read** admitting a caller who holds an active session on that visit, rather than a widened rule - so a guest's phone still cannot `list` the visit. Line-to-guest attribution was refused for `RD-TS-12`'s reason: nobody asked to be shown what the person opposite them ate.
- Can a guest create a Bite from an order without a BiteTribe account? Still open. The proposal is no, because it is the natural account-upgrade moment, and issue [#1657] now makes that upgrade keep the guest's uid so a prefilled draft survives it for free.
- What happens to the prefilled Bite if the guest never publishes it? Still open. The proposal is to offer it once at visit close and once in a notification, then drop it.
- ~~Are prices from a real order trusted without the suspicious-price validation from issue [#967]?~~ **The question dissolves: there is no such validation.** Issue [#967] shipped `Validators.required` and a float-format validator on the Bite form, which is a format check rather than a plausibility one. An ordered price has nothing to bypass. If a plausibility check is added later, an ordered price is the case it should exempt.
...
Moderation questions raised by threading are recorded on [epic-1284](../records/epic-1284.md), not here.
...
- What verified request ratio is acceptable before enabling App Check enforcement? **Settled.** The question never became a gate: server-side enforcement has been active for Firestore, Storage and Authentication since before Run 4, and Places API (New) stays in Monitoring by decision under [#1245]. See [Current State - Release State](release-state.md).
- Which Android and iOS devices are required for launch testing? **Settled** by the charter: a physical Samsung SM-A566B on Android 16 and an iPhone 12 mini on iOS 26.6, plus web. See [Current State - Release Candidate Test Charter](release-candidate-test-charter.md).
- What store screenshots and copy are needed for App Store and Google Play? **Settled** per slot in [Implementation - Store Listing Assets](../implementation/store-listing-assets.md), including which gaps are accepted rather than filled.
...
These belong to [epic-1487](../records/epic-1487.md), the city landing pages. Each gates the first published page rather than the first commit, and two of them are not engineering questions.
