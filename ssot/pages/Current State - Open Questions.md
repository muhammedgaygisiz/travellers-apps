...
- Is an order shared by the table or separated by guest? Still open, and now the only unsettled half of the sharing question: the _session_ is per guest and the _visit_ is shared, so an order line can be attributed either way without a migration. It belongs to issue \#1103.
...
- How are table changes handled after ordering has started? Still open for the guest-facing half. The backend half holds: `syncTableSessions` finds a visit's sessions by `visitId` rather than by table, so a party moved by `moveTableVisit` keeps its guests. What a guest is _shown_ when their table changes under them is unowned.
...
- **Where does a restaurant turn table ordering on?** Issue \#1100 added `Restaurant.tableOrdering` - `enabled`, the `timeZone` the opening hours are written in, and a staff-side `pausedUntilTimestamp` - and made the scan resolution gate on it. Nothing writes it, so every scan in production is refused with `tableOrderingDisabled` until something does. **Issue \#1102 is the nearest owner**, since its "menu-only mode for restaurants that publish a QR menu but do not enable ordering" is this flag seen from the guest's side - but it describes the mode rather than the surface that sets it. The open part is whether the switch belongs on the restaurant profile page, on the floor-plan page next to the QR sheets, or on a surface of its own that the pause belongs to as well - the pause is a staff action during service and the other two are owner configuration, so one page may not be the right home for all three. Issue \#1101 added a third field to the same object, `sessionIdleTimeoutMinutes`, with the same problem: nothing writes it either, and it is owner configuration rather than a service-time control.
- **What should a scan at a menu-only restaurant do?** Today `tableOrdering.enabled: false` is a dead end: `resolveTableQrToken` refuses with `tableOrderingDisabled` and tells the guest to ask a member of staff (issues \#1100, \#1101). Issue \#1102 wants the opposite - "menu-only mode for restaurants that publish a QR menu but do not enable ordering", whose whole point is that the guest still reads the menu. So `tableOrderingDisabled` has to stop being a refusal and become a branch: a resolved scan carrying the menu and no way to order. That changes a member of the closed refusal list and the shape `TableScanResult` returns, which is \#1102's work and is named here so it is not met as a surprise.
