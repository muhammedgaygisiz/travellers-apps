...
## Badges And QR Codes In The Web App

The web app carries a route to both listings, added by [Issue #1453][#1453]. It
is the surface a link in a message or a search result lands on, and until then
the repository held no store link anywhere.
...
All five live in `apps/bite-tribe/src/assets/store/`.

| File                        | Source                                                                 |
| --------------------------- | ---------------------------------------------------------------------- |
| `app-store-qr.svg`          | Generated by `tools/generate-store-qr-codes.mjs`                       |
| `google-play-qr.svg`        | Generated by `tools/generate-store-qr-codes.mjs`                       |
| `app-store-badge-black.svg` | Apple Marketing Tools, `Download_on_the_App_Store_Badge_US-UK_RGB_blk` |
| `app-store-badge-white.svg` | Apple Marketing Tools, `Download_on_the_App_Store_Badge_US-UK_RGB_wht` |
| `google-play-badge.png`     | Google Play badge generator, `en_badge_web_generic.png`                |

The two codes encode the listing URLs built from the identities in
[Console Coordinates](#console-coordinates) - Apple ID `6746098595` and package
`com.bitetribe.app`. They are committed rather than drawn at runtime, so the web
bundle carries no encoder for two URLs that cannot change without a new listing.
**A change to either identity means re-running `npm run generate-store-qr-codes`
and committing what it writes**; nothing checks that the committed code still
matches the constant beside it.

The three badge files are Apple's and Google's own artwork and must not be
recoloured, cropped, retyped or redrawn. Both store guidelines permit the
supplied badge and no derivative of it. Apple ships the pair, black for light
backgrounds and white for dark, and the component picks between them on the same
two conditions the app's own theme switches on; Google ships one badge, which its
guidelines place on either.

Both badges are shown at the same 40px of visible artwork. The clear space each
one needs is a tenth of its height and reaches them differently: Apple's SVG is
the badge alone, so the component adds the margin, while Google's PNG is a 646x250
canvas around a 564x168 badge and already carries it.

### Which Control, And Why

Above 600px each listing is a QR code; at or below it, the store's own badge. The
switch is a viewport media query rather than anything in TypeScript, because it
is a property of the viewport and CSS observes that directly.

The reason for the split is that neither control works at both sizes. On a
desktop browser a plain store link opens a listing on the wrong device, which is
what makes a code the right control there. On a phone browser a code cannot be
scanned by the screen displaying it, which is what makes it useless there.

The badges carry the English store names in all eleven locales. Apple and Google
both publish localized badge artwork, and adopting it would mean 22 files and a
locale-to-asset map rather than three files and none; that trade has not been
made.
