...
Execution - Build 96 (Run 10)" of [Current State - Release Candidate Test Charter](../current-state/release-candidate-test-charter.md),
...
Defects filed: [#1414] `P0`; [#1411] `P2`; [#1412], [#1413], [#1415], [#1416], [#1417], [#1418] `P3`
...
The gate's own layout is wrong, which is [#1411].
...
| [#1395] legacy image sweep       | **Not verifiable on this device.** See [#1413] |
...
An in-place Play update from 95 to 96 would most likely have migrated them. But [#1395] exists precisely because these files _survive uninstall_, and files that survive an uninstall are exactly the ones the app can never see again. Filed as [#1413].
...
**[#1414] `P0` - gallery photo GPS is never read on Android.**
...
Resolved by pinning the picker to `8.0.2`, with a `pickFiles` fallback for the OEM `file://` case the pin reintroduces. See the Gallery Picker Version Pin in [Architecture - Capacitor](../architecture/capacitor.md).
...
| ------- | -------- | ------------------------------------------------------------------------------------- |
| [#1414] | `P0`     | Gallery photo GPS never read on Android; the photo-location step delivers nothing     |
| [#1411] | `P2`     | App Check retry gate is top-aligned and collides with the status bar                  |
| [#1412] | `P3`     | Onboarding location step ignores an already-granted permission                        |
| [#1413] | `P3`     | Legacy local-image migration cannot see files owned by a previous install             |
| [#1415] | `P3`     | Language picker names Portuguese by exonym, breaking the endonym convention           |
| [#1416] | `P3`     | Turkish settings strings drop their diacritics; theme label wrong in `tr`, `de`, `fr` |
| [#1417] | `P3`     | My Bites shows the discovery feed's "Be the first one" empty state                    |
| [#1418] | `P3`     | Local image copies survive Bite deletion and account deletion                         |
...
[#571]: https://github.com/muhammedgaygisiz/travellers-apps/issues/571
...
[#1403]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1403
[#1410]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1410
...
[#1415]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1415
