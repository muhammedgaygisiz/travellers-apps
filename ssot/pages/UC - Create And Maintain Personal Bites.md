...
- The photo comes from the camera or from the gallery, and on Android the gallery is the system Photo Picker: a surface outside the app that returns the one photo the user selected. It is opened in single-select mode, so the choice is a tap rather than a checkbox and a confirmation.
- The Bite form keeps the media location permission and asks for it in front of that picker, even though the picker needs none. Android strips a picked photo's location metadata for a caller without the grant, and `Aus Bild` is the source that makes posting a photo taken earlier work at all, so the permission prompt is paid for the position. Under "Allow limited access" the OS then asks the user to name the photo before the picker does — a double selection that cannot be removed while the photo position is wanted, because the prompt and the readable EXIF are the same grant. See GitHub issue #1394.
