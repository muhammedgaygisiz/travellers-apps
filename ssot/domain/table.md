...
- **A scan resolves to a Table only if the Restaurant behind it is one worth showing.** The Restaurant exists and is held by a business account, the Table is in the published plan and in service, the token is active, the Restaurant is open, and the menu has something on it. A scan that fails one of them is answered with which one, not with a generic error - a guest at a table who is told "something went wrong" puts their phone away. See [UC - Order At The Table Through A QR Code](../use-cases/uc-order-at-the-table-through-a-qr-code.md) and issue [#1100].
- **Whether the Table can be _ordered_ from is a separate answer** (issue [#1102], `RD-TS-6`). Table ordering being switched off, and staff having paused it, used to refuse the scan; they now resolve it and say ordering is shut, because neither is a reason to withhold a menu. A Restaurant that is closed still refuses: that is a statement about the Restaurant rather than about ordering.
...
- Live state is not part of the Table. It lives separately, so operational updates never rewrite the layout. See [Table Visit](table-visit.md).
...
A Table State carries `tableId`, `restaurantId`, `status`, `since`, `updatedByUserId`, an optional `visitId` naming the open [Table Visit](table-visit.md) while a party is seated, and an optional free-text `note`. `since` is the moment of the last accepted transition rather than the last write, because what a host reads off the screen is a duration: table 6 has been awaiting payment for twenty minutes.
...
the table itself, and it is documented on [Table Visit](table-visit.md) with the visit it
attaches to.
