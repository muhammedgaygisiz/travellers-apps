# issue-1409 - Move the media location permission into onboarding

Closing record for [#1409].
Written before the [Closing Report](../overview/github-issue-format.md#closing-report) rule; kept as-is, not extended.

## Description

- Follow-up to [issue-1394](issue-1394.md), which is where the finding and the reasoning live. That issue asked for the Android Photo Picker on the gallery path so attaching a photo raises no prompt and, under "Allow limited access", costs one selection instead of two.
...
- Onboarding is already the repo's primary first-run request surface, and the Contextual Permission Rule on [Architecture - Capacitor](../architecture/capacitor.md) already routes push and location through it. Media location was the only OS permission still being requested from the middle of a task.
...
- Recorded as a rewritten Media Permission Rule on [Architecture - Capacitor](../architecture/capacitor.md), an extension of its Contextual Permission Rule, and flow bullets on [UC - Create And Maintain Personal Bites](../use-cases/uc-create-and-maintain-personal-bites.md).
