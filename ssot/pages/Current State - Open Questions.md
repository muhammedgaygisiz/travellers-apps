...
- How are table changes handled after ordering has started? Still open for the guest-facing half, and sharper since issue \#1104. The backend half holds: `syncTableSessions` finds a visit's sessions by `visitId` rather than by table, so a party moved by `moveTableVisit` keeps its guests, and the guest's order listener follows the visit rather than the table - so a moved party goes on seeing its orders. What is wrong is the heading above them: the screen names the table off the scan, and a guest walked to another table reads their old table number on their own phone for the rest of the meal. Still unowned.
...
- How are cancelled or incorrect orders corrected? Still open, and narrower. Issue \#1103 fixed the shape of the answer without giving it: the five order statuses and the transitions between them are declared as data, `cancelled` is reachable from every live status because a kitchen that runs out halfway has to be able to say so, and `firestore.rules` refuses every client write to an order - so a correction can only ever be a staff action. Issue \#1104 then settled the **guest-facing** half by `RD-TS-13`: `TableOrder.cancellationReason` exists, the guest's screen renders it, and an order cancelled without one is still named as cancelled rather than shown silently. **Who may cancel, and whether a reason is required rather than merely possible, is issue \#1105's.**
