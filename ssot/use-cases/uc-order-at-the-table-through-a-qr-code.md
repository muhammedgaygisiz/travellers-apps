...
- A guest scans **as whoever they already are**, and only a signed-out one is signed in anonymously. `signInAsGuest` returns the existing user when there is one and mints an anonymous account otherwise, and `guestOf` in `start-table-session.ts` admits any authenticated caller, reading `isAnonymous` off the token's `sign_in_provider` - so a member who scans a code at a table orders as themselves, and signing in again would mint a second uid and orphan both sessions. An anonymous account is not a member (`RD-TS-4`): the uid is what the rules match the guest's own session document against, it writes no `/users` document, and it does not pass the app's route guards. **That anonymous guest is upgraded in place when they register**, delivered by issue [#1657] and recorded as `RD-TS-41`: all three sign-up paths reach `linkWith*` while the session is anonymous, so the uid does not change and the meal stays theirs; the link then forces a token refresh, because the old one still says `anonymous` to `isMember()` and `requireMember` (`RD-TS-40`), and calls `upgradeGuestAccount` for the `/users` document the blocking trigger never writes for a link. The guest who already **has** an account takes the other path, delivered by issue [#1658] and recorded as `RD-TS-42`: Firebase refuses the link, and the refusal becomes an offer - sign in, and `claimTableVisit` moves the session, the orders and the assistance signals onto that account in one transaction, then deletes the anonymous one. What authorises the move is an ID token of the anonymous session taken _before_ signing in, verified server-side and required to be an anonymous session's; declining the offer leaves the guest ordering exactly as they were. Where the account already held a session at that table the two are merged under `RD-TS-42`'s rules, and two live sessions in different visits are refused rather than merged. See [User Roles](../product/user-roles.md).
...
model, the five statuses and the storage are on [Table Visit](../domain/table-visit.md).
...
[#344]: https://github.com/muhammedgaygisiz/travellers-apps/issues/344
