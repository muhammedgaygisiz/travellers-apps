# issue-1389 - bug: Restaurant page tags are not deduplicated across the # prefix and case

Closing record for [#1389].
...
- Recorded as a rule in [Restaurant](../domain/restaurant.md).
...
- **Case folding is not locale-aware.** `toLowerCase()` is used so the result does not depend on the host locale, which means Turkish `İstanbul` and `istanbul` are still two tags. See [issue-1388](issue-1388.md) for why locale-aware casing needs the language passed in explicitly.
