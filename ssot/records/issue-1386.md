# issue-1386 - bug: Push notification settings do not reflect or recover the OS permission on Android

Closing record for [#1386].
...
- **Only `granted` means delivery.** Every other state mutes the device, and the difference between `denied` and `prompt` selects the recovery action rather than whether to show one. This is the fix; the rest follows from it. Recorded as the OS Permission Reflection Rule in [Architecture - Capacitor](../architecture/capacitor.md).
