# issue-1031 - 06 - chore: update Angular, NgRx, and Ionic within current majors

Closing record for [#1031].
...
- Ionic `8.7.18`+ patches `childNodes`/`textContent` under jsdom, which required reworking one snapshot and two `textContent` assertions. Browser behavior is unaffected. See [Current State - Known Issues](../current-state/known-issues.md).
...
- Details and the validation run are recorded in [Current State - Nx And Dependency Migration Roadmap](../current-state/nx-and-dependency-migration-roadmap.md).
