...
- Can guests order without a BiteTribe account? **Settled 13 September 2026 by `RD-TS-4`:** yes, through anonymous authentication, upgradeable in place. An anonymous account is deliberately not a member: it writes no `/users` document and does not pass the route guards. **Reopened 17 September 2026 by issue [#1629]**, which decides whether anonymous sessions stay allowed and how the use case names their actor. What such a session may _reach_ is settled separately and either way, by `RD-TS-37` and `RD-TS-38` on issue [#1565].
- **What may an anonymous session reach beyond the table it scanned? Settled 20 September 2026 by `RD-TS-37` and `RD-TS-38`, owned by issue [#1565] until it is delivered.** Nothing: `isMember()` takes the fifteen read clauses written for members, and the nineteen member callables refuse a token whose sign-in provider is `anonymous`. Until that lands, a uid minted by photographing a sticker reads every `/users` document and spends billed Google Maps quota, which is why the issue carries the `security` label rather than waiting on [#1629].
