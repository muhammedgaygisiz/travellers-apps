...
- **The platform test pass ([issue 1176](https://github.com/muhammedgaygisiz/travellers-apps/issues/1176)) is incomplete.** The web half passed at Run 8 and the iOS half at Run 7, each against a named build. **Android was executed for the first time on 27 August 2026 as Run 9, against Play Open Testing build 95, on a physical Samsung SM-A566B running Android 16, and it failed on one `P0`.** [Issue #1385](https://github.com/muhammedgaygisiz/travellers-apps/issues/1385): the Android client resolves the sign-in provider to `unknown` for an email/password account, so it can never answer the backend's `reauth_required` and account deletion cannot complete. The tester also ran a deletion inside the backend's five-minute recency window and it failed there too, so the re-auth path is not the only fault and the root cause is not yet fully bounded. The re-auth half is now understood and fixed in the client - see [[issue-1385]] - but nothing has been run on a device since, so the blocker stands until a build proves both halves. Store policy and GDPR both require a working deletion, so the release candidate cannot ship on it. The run was stopped there by decision, so the Android half is a fail **and** incomplete; eleven issues were filed in total, [#1385](https://github.com/muhammedgaygisiz/travellers-apps/issues/1385) to [#1395](https://github.com/muhammedgaygisiz/travellers-apps/issues/1395). Checks 1 to 7, 9, 11, 13 and the accepted-token half of 12 passed. The artifact half of that blocker cleared on 23 August 2026: build 95 was released to TestFlight and Google Play Open Testing, replacing the build 87 the Android track had carried since 26 July. Nothing has been executed against it yet, so the run itself is still outstanding, but it is now waiting on someone rather than on a distributable build. Notification delivery and a Crashlytics non-fatal from Android remain unverified. DebugView is now worse than unverified on Android: [issue #1387](https://github.com/muhammedgaygisiz/travellers-apps/issues/1387) found native analytics collection disabled on the test device, so it is recorded as an evidence gap rather than a pending check. That cause is now confirmed and fixed - the native SDK persists the flag a dev build wrote, and production now asserts it back - but the repair only exists in a build carrying the fix, so build 95 cannot close the gap. See [[issue-1387]]. **The enforced App Check gate (charter check 12) passed its accepted-token half for the first time**, proven from production Cloud Function logs showing `{"verifications":{"app":"VALID","auth":"VALID"}}` against build 95, the first artifact built with `NX_APP_BITE_TRIBE_APP_CHECK_ENFORCED=true`. The refused-token retry gate is still unexercised on every platform.
...
- Android testing completed. **Executed 27 August 2026 (Run 9), failed on [#1385](https://github.com/muhammedgaygisiz/travellers-apps/issues/1385) and stopped incomplete.**
