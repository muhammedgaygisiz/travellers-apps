...
Execution - Build 95 (Run 9)" of [Current State - Release Candidate Test Charter](../current-state/release-candidate-test-charter.md),
...
Defects filed: [#1385] `P0`; [#1386], [#1387] `P1`; [#1388], [#1389], [#1390] `P2`; [#1391], [#1392], [#1393], [#1394], [#1395] `P3`

## Entry State And Build Identity

- Build under test: the Google Play Open Testing artifact, `com.bitetribe.app` **1.0.1 (95)**, installer `com.android.vending`, delivered as split APKs. Tag `build-1.0.1-95` is commit `f2ae6dbf`, 23 August. The in-app menu, the About page and the Settings device row all report `1.0.1 (Build 95)`, so [issue #1303][#1303]'s version disagreement, found on iOS, does **not** reproduce on Android.
...
This is the direct contrast with iOS, where run 6 session 15 recorded that disabling the permission in iOS Settings was recognised, explained, and given an action that opened the right Settings page. **[Issue #1184][#1184]'s OS-permission recovery route exists on iOS and does not exist on Android.** Filed as [#1386]. Delivery of a ranking-change notification remains unverified, and [#1386] is why.

**Since fixed, and not yet verified on a device.** The blocked state, the recovery action and the copy for both already existed - none of it appeared on Android because only `denied` counted as muted, and Capacitor returns a `POST_NOTIFICATIONS` revoked in system settings to an unspent `prompt`. The three symptoms are one assumption: the row's switch now shows what actually arrives rather than the stored `enabled` flag, a muted device is explained, switching it back on asks the OS instead of writing Firestore, and Android has a settings route at last - a native intent in the app's own wrapper, because App Launcher has no URL for it. The reasoning is in [issue-1386](../records/issue-1386.md). This check is the test that closes it, and it also unblocks the ranking-change delivery check waiting behind it.
...
| ------- | -------- | --------------------------------------------------------------- |
| [#1385] | P0       | Account deletion fails outside the five-minute re-auth window   |
| [#1386] | P1       | Push settings do not reflect or recover the OS permission       |
| [#1387] | P1       | Native analytics stays disabled on devices that ran a dev build |
| [#1388] | P2       | Turkish uppercase drops the dotted İ                            |
| [#1389] | P2       | Restaurant tags not deduplicated across the `#` prefix and case |
| [#1390] | P2       | Offline photo upload retry never times out                      |
| [#1391] | P3       | A typed Bite tag is lost unless a space is pressed              |
| [#1392] | P3       | Bitemap drawer has no bottom safe-area spacing                  |
| [#1393] | P3       | Action sheet lists Cancel before the real actions               |
| [#1394] | P3       | Use the Android Photo Picker for the gallery path               |
| [#1395] | P3       | Legacy unscoped image copies remain in public Documents         |
...
**Release-candidate fail for the Android half**, on one P0, [#1385]. The run was stopped at that point by decision rather than completed, so the Android half is also incomplete.
...
- 115 legacy image copies remain in public `/sdcard/Documents`, which is [#1395].
...
[#1384]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1384
...
[#1388]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1388
[#1389]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1389
[#1390]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1390
