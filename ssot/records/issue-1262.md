# issue-1262 - bug: default currency is suggested from the language variant, not the device region

Closing record for [#1262].
Written before the [Closing Report](../overview/github-issue-format.md#closing-report) rule; kept as-is, not extended.

## Description

- Found in iOS release-candidate Run 5 under [Current State - Release Candidate Test Charter](../current-state/release-candidate-test-charter.md), Session 5. The onboarding currency step prefilled `British Pound` on an iPhone whose iOS Region is `Switzerland`, under copy reading "We picked one from your device".
