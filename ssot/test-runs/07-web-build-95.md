...
Execution - Build 95 (Run 7)" of [Current State - Release Candidate Test Charter](../current-state/release-candidate-test-charter.md),
...
- **This is also the first concrete evidence on App Check's web path**, which [Current State - Known Issues](../current-state/known-issues.md) lists as an outstanding gap: attestation succeeds for a legitimate client and correctly refuses a suspicious one. It does not close the enforced-mode gap, which still needs a build with the flag on.
...
- **Two findings came out of this page**, both filed and both `P3`:
  - [Issue #1330][#1330]: the searchbar spans the full viewport on desktop. Measured at 1470px wide: searchbar **1408px**, Bite card 459px, with `max-width: none`. It is roughly three times the width of a card and ignores the measure every other element respects. Invisible at phone widths, conspicuous on desktop.
  - [Issue #1331][#1331]: a search matching nothing shows the empty-**feed** copy, `No bites found. Be the first one.`, telling a user who has Bites that none exist and inviting them to create one, when the correct action is to change the search term.
...
- **Deleting the Bite captured the toast the iOS half missed.** The confirmation alert reads `Delete Bite` / `Are you sure you want to delete this bite?`, and the result was `ion-toast`, `message: "Bite deleted successfully!"`, **`color: "success"`**, **`position: "top"`**. Run 5's burst-capture failure left the iOS delete toast recorded on the tester's word alone; this is the structured evidence. [Issue #1305][#1305]'s success path is now verified on two distinct actions, on web, with attributes rather than inference.
- **[Issue #1310][#1310] decrements on web too**: the profile went from `1` to **`0`** with no reload, matching the iOS behaviour. The fix is bidirectional on both platforms.
- Incidentally this validated [issue #1331][#1331]'s framing rather than contradicting it: after the delete, `No bites found. Be the first one.` is **correct**, because the account genuinely had no Bites. The copy is only wrong when a filter caused the emptiness.
...
- Filed as [issue #1328][#1328], `bug` plus `security`, and **the run's most consequential finding**.
...
- Filed as [issue #1327][#1327]. The `/home` feed instantiates **50 `ion-alert` elements**, every one headed `Delete Bite`, against 51 `ion-card` elements, all carrying `overlay-hidden`, out of 2305 DOM nodes.
...
- **Result: fail, on two P0 findings, with six findings in total.** [#1326] and [#1327] were **accepted as release blockers by decision** after the run. [#1328] is `P1`, [#1329] is `P2`, and [#1330] and [#1331] are `P3`. Build 95's web deploy is therefore not a release candidate.
...
- Getting there is worth recording, because the Bite id had died with its document. It was recovered from the **local gallery**, using the very behaviour [issue #1328][#1328] reports: the gallery survives logout and account deletion, and its filenames follow `bites_<docId>.<ext>`, so the surviving `bites_9afeaf5e-….jpg` still named the deleted Bite. The file was confirmed as ours by its dimensions — 800x600, the exact canvas size the placeholder was generated at.
...
[#1325]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1325
[#1326]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1326
[#1327]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1327
...
[#1330]: https://github.com/muhammedgaygisiz/travellers-apps/issues/1330
