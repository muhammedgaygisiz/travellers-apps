# issue-1412 - bug: Onboarding location step ignores an already-granted permission

Closing record for [#1412].
Written before the [Closing Report](../overview/github-issue-format.md#closing-report) rule; kept as-is, not extended.

## Description

- Found in [Current State - Release Candidate Test Charter](../current-state/release-candidate-test-charter.md) Run 10, the Android re-run, on a locally built debug artifact of `develop` at `78889774`, build 1.0.1 (96), on a physical Samsung SM-A566B running Android 16. Run 9 had recorded the same behaviour on iOS as a cross-platform observation rather than an issue.
...
- Recorded as the Live State Rule on [Architecture - Capacitor](../architecture/capacitor.md), as a target-flow rule on [UC - Guide New Users After Registration](../use-cases/uc-guide-new-users-after-registration.md), and traced on [Traceability Map](../overview/traceability-map.md).
