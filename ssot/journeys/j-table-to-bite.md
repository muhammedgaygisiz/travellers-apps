...
| #   | Lane     | Step                                                          | Surface or callable                            |
| --- | -------- | ------------------------------------------------------------- | ---------------------------------------------- |
| 01  | Operator | Assigns the restaurant to a business account                  | `assignRestaurantOwner`                        |
| 02  | Backend  | Rules scope every write to the owner                          | `firestore.rules`                              |
| 03  | Owner    | Lays out rooms, walls and tables in millimetres               | `/restaurant/:id/floor-plan`                   |
| 04  | Owner    | Gives each table a label, seats and an enabled state          | same                                           |
| 05  | Owner    | Validates and publishes the plan                              | same                                           |
| 06  | Backend  | Issues an opaque token for every enabled table                | `/tableTokens/{token}`                         |
| 07  | Owner    | Prints QR sheets as table tents or a sticker grid             | `/restaurant/:id/floor-plan/qr-codes`          |
| 08  | Owner    | Turns table ordering on, and names the time zone              | `/restaurant/:id`                              |
| 09  | Staff    | Signs in and lands on the live room view                      | `/restaurant/:id/tables`                       |
| 10  | Staff    | Seats a party, which opens the visit                          | `transitionTableState`                         |
| 11  | Staff    | Marks tables reserved, cleaning or disabled                   | same                                           |
| 12  | Staff    | Moves a party to another table                                | `moveTableVisit` - **no surface calls it**     |
| 13  | Backend  | Freeing a table closes its visit, landing on `cleaning`       | `transitionTableState`                         |
| 14  | Guest    | Scans the code on the table                                   | `/t/:token`                                    |
| 15  | Backend  | Resolves the token, or refuses with one of ten reasons        | `resolveTableQrToken`                          |
| 16  | Guest    | Confirms "You are ordering at Sakura Kitchen, table 12"       | `/t/:token`                                    |
| 17  | Backend  | Opens the session under whoever the guest already is          | `startTableSession`                            |
| 18  | Staff    | Seats the party, which confirms every guest waiting on it     | `transitionTableState`                         |
| 19  | Guest    | Reads the menu, sold-out dishes marked and not addable        | `/t/:token/order`, or `/m/:id` with no account |
| 20  | Guest    | Builds a cart and submits the order                           | `submitTableOrder`                             |
| 21  | Staff    | Works the order queue and moves each status                   | `/restaurant/:id/orders`                       |
| 22  | Guest    | Watches each order move, and orders again into the same visit | `/t/:token/order`                              |
| 23  | Guest    | Asks for help, or asks for the bill                           | `requestTableAssistance`                       |
| 24  | Staff    | Acknowledges the call                                         | `acknowledgeTableAssistance`                   |
| 25  | Guest    | Pays in the app, or asks staff to settle                      | not built, [#1110]                             |
| 26  | Staff    | Settles the bill and closes the visit                         | not built, [#1111]                             |
| 27  | Guest    | Sees a receipt of the dishes they ordered                     | not built, [#1111]                             |
| 28  | Guest    | Picks a dish and publishes a Bite                             | not built, [#1112]                             |
| 29  | Backend  | Links menu item and Bite in both directions                   | not built, [#1113]                             |
