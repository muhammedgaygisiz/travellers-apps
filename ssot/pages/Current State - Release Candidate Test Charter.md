...
3. **[#1184](https://github.com/muhammedgaygisiz/travellers-apps/issues/1184): Push notification settings cannot be changed.** iOS notifications and the stored app preference were on, but the Push Notifications toggle on the app Settings page was disabled. The issue is respecified to replace the account-wide preference with per-installation token controls, persistent installation identity, and an explicit current-device setup and OS-permission recovery flow. Implementation and physical iOS/Android verification remain open.
