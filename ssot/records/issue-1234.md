# issue-1234 - bug: Account deletion confirmation does not identify the account

Closing record for [#1234].
Written before the [Closing Report](../overview/github-issue-format.md#closing-report) rule; kept as-is, not extended.

## Description

- iOS release-candidate Run 3 for [issue 1176](https://github.com/muhammedgaygisiz/travellers-apps/issues/1176) on TestFlight 1.0.1 build 90 completed the whole deletion contract from [issue-1182](issue-1182.md) within two minutes, including password reauthentication, progress, and the return to the unauthenticated Start screen.
...
- Reauthentication is untouched. The recent-sign-in contract from [issue-1182](issue-1182.md) still decides when a password or a provider sheet is required; the prompt only gained the account it belongs to.
...
- Not run: the Playwright account-and-legal spec, which needs the Firebase emulator, and the physical-iOS recheck the issue's acceptance criteria require before this can be recorded under [Current State - Release Candidate Test Charter](../current-state/release-candidate-test-charter.md).
