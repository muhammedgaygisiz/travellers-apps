...
- A Table is a business entity, not a shape. See [Table](table.md).
...
geometry" a structural fact rather than a promise. See [Table](table.md) for the label
...
**Publishing asks for the QR codes.** [Table](table.md) gives every enabled table an
...
- The rules deploy from CI on every push to `develop` ([#1567]), so the floor-plan rules mean something in production from the merge - including, since issue [#1086], the public read on `/tableTokens` and the backend-only `qrTokenId` a scanned code is resolved through. This was a workstation command until [#1567], and an undeployed rules file left a scanned code unresolvable.
