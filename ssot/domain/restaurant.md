...
- **A menu item can be referenced now, and its currency still cannot be read.** Issue [#1099] gave `MenuItem`, `Category` and every variant an `id`, generated once and never reused, and issue [#1598] extended that to every extra, and made the business editor and both renderers key by it rather than by `title` and `name` - so renaming a dish or reordering a category no longer moves the entry anything else points at. `backfillMenuItemIds` on the admin migrations surface fills in menus written before that, and the client fills in whatever it still finds missing on read. What a menu still carries nowhere is a currency: the consumer menu hardcodes a euro sign, the business editor labels the price with a dollar sign, and `MenuService.prepareBiteFromMenuItem` hardcodes `EUR`. `OrderLineSnapshot` names the field an answer has to land in; choosing the source is issue [#1103].
- **A category's extras are authorable, readable and orderable.** Issue [#1598] finished `Category.extrasBlock`, which had carried extras since before stage 3 with nothing rendering them and no way for an owner to write one. An extra now has an `id` on the same terms as an item's - filled for older menus by the same `backfillMenuItemIds` migration and by the client on read - and `RD-TS-35` settles that it belongs to the **category**, applying to every dish printed under it, with `menuExtrasForItem` the one place that rule is applied. The business editor has an extras section per category with add, rename, reprice and remove; the consumer and public menus print the block under the dishes; the ordering screen replaces it with a tick box per orderable dish. A submitted line records each extra by id, name and price, `submitTableOrder` revalidates each against the live menu, and `extraMissing` and `extraPriceChanged` name the extra and both prices when one has been withdrawn or repriced mid-meal (`RD-TS-36`). What is still not expressible is an extra offered on only some items of a category, or any option group with a cardinality rule.

- **A scanned table code resolves, and an owner can turn table ordering on.** Issue [#1100] added `resolveTableQrToken`, which validates the rules of [UC - Order At The Table Through A QR Code](../use-cases/uc-order-at-the-table-through-a-qr-code.md) and answers with a restaurant, a room, a table and a menu, or with one of ten distinct refusal reasons. `Restaurant.tableOrdering` is written by the business app's restaurant page as of issue [#1102] - `enabled` and the IANA `timeZone`, with a staff-side pause carried through rather than set, since no surface starts one. Ordering being off is no longer a refusal: `tableOrderingDisabled` is an ordering verdict on a scan that _resolved_ (`RD-TS-6`), so a restaurant that takes orders from a person with a notepad still shows the guest its menu - and since issue [#1597] it shows it even when every dish is marked off.
